...
class TurtleControllerNode(Node): # MODIFY NAME
    def __init__(self):
        ...
        self.psoe_subsriber_ = self.create_subscription(Pose, "turtle1/pose", self.callback_turtle_pose, 10)
    ...
    def callback_catch_turtle(self, future, turtle_name):
        try:
            ...
            if not response.success:
                self.get_logger().error("Trutle " + str(turtle_name) + " could not be caught")
        except Exception as e:
            self.get_logger().error("Serwice call failed %r" % (e,))
